...
import { setshowpanel2 } from '@/Store/Slices/adminslice';
interface Props {
    Styles: StyleType[];
    Masters: MasterType[];
}
export default function Panel2(props: Props) {
    let { Styles, Masters } = props;
    ...
    let show = useSelector((state: RootState) => state.example.showpanel2);
            ...
            <div className="w-1/2 h-fit bg-white opacity-100 relative">
                ...
                <div className="w-full flex justify-center">
                    <div className="flex flex-row w-3/5 justify-between">
                        <div>
                            <h4 className="font-sans font-[400] text-[24px]  ">
                                Стили{' '}
                            </h4>
                            <select
                                id="cars"
                                className="border border-black w-[150px] text-center  bg-[#d9d9d9] flex justify-around"
                                ref={selectRef1}
                            >
